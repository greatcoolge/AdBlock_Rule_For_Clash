...
foreach ($url in $urlList) {
    ...
    try 
    {
        ...
        {
            # 第一种情况：加上前缀 '+.' 和后缀 '' 的情况
            if ($line -match '^(\|\|.*\.example\.com\^$|example\.com|/^\[a-z0-9-]+\.example\.com$/|address=/example\.com/127\.0\.0\.1|address=/example\.com/0\.0\.0\.0)' -or $line -match '\|\|example\.com\^\$all') {
                $formattedRule = "+.$line+"
                $uniqueRules.Add($formattedRule) | Out-Null
            }
            # 第二种情况：加上前缀 '' 和后缀 '' 的情况
            elseif ($line -match '^\|\|example\.com\^$' -or $line -match '^/^\|\|example\.com\^$' -or $line -match '127\.0\.0\.1 example\.com' -or $line -match '0\.0\.0\.0 example\.com') {
                $formattedRule = "$line"
                $uniqueRules.Add($formattedRule) | Out-Null
            }
            # 第三种情况：忽略条目
            elseif ($line -match '^@@\|\|example\.com\^' -or $line -match '\|\|example\.com\^\$') {
                continue
            }
        }
    }
